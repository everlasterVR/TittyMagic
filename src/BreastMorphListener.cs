...
{
    // adapted from MorphMassManager v7 by nyaacho
    // https://hub.virtamate.com/resources/morph-mass-manager.4013/
    class BreastMorphListener
    {
        private List<int> breastVertices = new List<int>() {
        ...
        private List<DAZMorph> listenedMorphs = new List<DAZMorph>();
        ...
        public BreastMorphListener(List<DAZMorph> morphs)
        {
            morphs
                .Where(morph => morph.visible
                    && !morph.group.Contains("Pose Controls")
        ...
        private void CheckMorphAndListen(DAZMorph morph)
        {
            ...
            {
                if(breastVertices.Contains(delta.vertex))
                {
                    hitDelta += delta.delta.magnitude;
                    if(!listenedMorphs.Contains(morph) && hitDelta >= hitDeltaMax)
                    {
        ...
        {
            return string.IsNullOrEmpty(morph.overrideName) ? morph.displayName : morph.overrideName;
        }
    }
}
