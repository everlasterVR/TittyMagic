...
namespace TittyMagic
{
    ...
    internal class BreastMorphListener
    {
        ...
        private float _filterStrength = 0.009f;
        ...
        private void CheckMorphAndListen(DAZMorph morph)
        {
            ...
            float sumDeltaMagnitudes = 0.0f;
            float sumAllDeltaMagnitudes = morph.deltas.Sum(x => x.delta.magnitude) * _filterStrength;
            foreach(var delta in morph.deltas)
            {
                ...
                sumDeltaMagnitudes += delta.delta.magnitude;
                if(!listenedMorphs.Contains(morph) && sumDeltaMagnitudes >= sumAllDeltaMagnitudes)
                {
